...
2. **创建 `docker-compose.yml` 文件**：
   ...
   ```
   *注意：如果你暂时没有 `config.yaml`，可以创建一个空文件或者使用项目提供的模板文件，以避免 Docker 将其识别为目录。如果你打算使用环境变量管理，可以创建 `.env` 文件。*

3. **启动服务**：
   ```bash
   docker compose up -d
   ```
   你可以通过 `docker logs -f cli-proxy-api` 查看启动日志。

### 3.2 部署注意事项与安全性配置

将代理暴露在公网的 VPS 上会带来极大的便利，但同时也伴随着较高的安全风险，特别是涉及到敏感的 OAuth Token 时。以下是你**必须注意的点**：

- **公网访问控制与反向代理 (Nginx)**：上述的 `docker-compose.yml` 已经将端口收缩至 `127.0.0.1`。现在我们需要通过 Nginx 暴露它并增加 `Basic Auth` 鉴权，以防止任何扫端口的脚本盗用你的额度。
  ...
  server {
      listen 80;
      server_name your_proxy.domain.com; # 替换成你的子域名
  ...
  ```
  *(建议搭配 Let's Encrypt 开启 HTTPS，确保 Token 在公网传输中的安全。)*
  
- **防火墙限制**：如果你不想折腾 Nginx 反代，直接在云服务商（如 AWS、阿里云海外等）的安全组或防火墙中，严格限制 `8317` 端口仅允许你本地的网络 IP 访问。
