---
title: OpenClaw 2026 年 2 月安全风暴与实战最佳实践（3 月增补版）
date: 2026-02-07
updated: 2026-03-01 01:00:00
tags:
  - OpenClaw
  - AI Skills
  - 安全
...
description: 2026 年 2 月，OpenClaw 生态经历了从恶意 Skills 供应链攻击到 ClawJacked 漏洞的全面安全洗礼。本文深度解析最新安全事件（含 CVE-2026-28363、Endor Labs 六大漏洞、Meta 邮件删除事件）、版本更新、VirusTotal 集成、MyClaw 托管方案，并给出可落地的安全加固最佳实践。
ai_generated: true
---

> 🤖 **AI 生成文章声明**：本文由 OpenClaw 自动化脚本抓取全球最新动态并深度分析生成。
> 📅 **实战数据更新**：2026 年 3 月 1 日（3 月增补版）

![OpenClaw Skills 生态概览](/images/openclaw/openclaw-awesome-skills-banner.png)

## 一、2 月安全风暴回顾：OpenClaw 的"成人礼"

2026 年 2 月对 OpenClaw 来说是历史性的一个月。项目 GitHub 星标突破 **22 万**，但随之而来的是一系列严肃的安全挑战，让整个社区进入了快速成熟期。Conscia 的安全研究团队将其总结为"**三周内爆发的多向量安全危机**"。
...
**🆕 Trend Micro 新发现（2 月下旬）**：Trend Micro 团队识别出 **39 个 Skills 通过操控 OpenClaw 安装伪造 CLI 工具来分发 Atomic macOS Stealer**。虽然这些 Skills 已被下架，但其代码仍存在于 ClawHub 的 GitHub 仓库中。Bitdoze 的独立调查也指出，**约 12% 的 ClawHub Skills 被感染了恶意软件**。
...
**2 月 27 日披露的最新严重漏洞。** 这是一个 `tools.exec.safeBins` 验证逻辑缺陷：
...
| 漏洞类型 | 说明 |
|---------|------|
| **SSRF（服务端请求伪造）** | 可被利用访问内部网络资源 |
| **身份验证缺失** | 部分端点未正确验证身份 |
| **路径遍历** | 通过规范化绕过访问敏感文件（已通过符号链接逃逸防护修复：`../../../etc/passwd` 不再有效） |
| **命令注入** | CVE-2026-24763 和 CVE-2026-25157 允许远程代码执行 |
...
基于 2 月的安全事件和 Microsoft、Cisco、Kaspersky、Endor Labs、Contabo 等安全团队的分析，整理以下可落地的加固建议：
...
*   **不要暴露 Gateway 到公网**：Kaspersky 报告指出大量 OpenClaw 实例暴露在互联网上。**Reddit r/sysadmin 最新热帖（900+ 赞）也在讨论这一问题**——大量用户在不了解镜像内容的情况下部署 OpenClaw
...
2 月下旬，**MyClaw**（myclaw.ai）作为 OpenClaw 的一键托管方案迅速走红：
...
同时，Contabo 等 VPS 提供商也推出了预配置的 OpenClaw 一键部署模板。社区博主在 Medium 上分享了"**4 分钟在 $5/月服务器上部署 OpenClaw**"的实战经验，使得自托管的门槛也在持续降低。

> 💡 **选择建议**：想省心用 MyClaw，想完全控制用 VPS 自托管。无论哪种方式，**都要遵循上面的安全加固清单**。

---

## 五、从"全能 Agent"到"原子化 Skills"：生态持续膨胀

尽管安全风波不断，OpenClaw 的 Skills 生态仍在加速膨胀。截至 2 月底 / 3 月初：

*   **GitHub 星标**：**22 万+**（较 2 月初的 21.5 万继续增长）
...
OpenClaw 创始人 Peter Steinberger（PSPDFKit 创始人）在 2 月宣布将加入 OpenAI，但 OpenClaw 项目将保持开源并移交至开源基金会运营。这一决定为项目的长期可持续发展提供了制度保障。

---

## 六、Vibe Coding：从概念到日常
...
### 🆕 与 Claude Cowork 和 Claude Code 的竞争格局

DEV Community 上的热门对比文章指出：
*   **OpenClaw**：最强的自主性和生态系统（3,000+ Skills），持续记忆，适合全场景 AI Agent
...
2 月是 OpenClaw 社区的分水岭。恶意 Skills 攻击、ClawJacked 漏洞、safeBins 绕过、Endor Labs 六大漏洞、Meta 邮件删除事件——每一个都在提醒我们：**个人 AI Agent 拥有的权限等级，要求我们以对待生产服务器的严肃态度来对待它。**

好消息是，OpenClaw 团队的响应速度极快——从 VirusTotal 集成到 40+ 安全修复，从 SSRF 默认策略升级到外部密钥管理，安全正在成为平台的一等公民。创始人将项目移交开源基金会的决定，也为生态的长期健康奠定了基础。
